...
from peekingduck.nodes.model.yolov6_core.utils.torch_utils import (
    time_sync,
    get_model_info,
)
...
class Evaler:
    ...
    def init_model(self, model, weights, task):
        if task != "train":
            ...
            LOGGER.info(
                "Model Summary: {}".format(get_model_info(model, self.img_size))
            )
    ...
    def eval_speed(self, task) -> None:
        ...
        if task != "train":
            ...
            pre_time, inf_time, nms_time = (
                1000 * self.speed_result[1:].cpu().numpy() / n_samples
            )
            for n, v in zip(
                ["pre-process", "inference", "NMS"], [pre_time, inf_time, nms_time]
            ):
    ...
    def eval_trt(self, engine, stride=32):
        ...
        def convert_to_coco_format_trt(
            nums, boxes, scores, classes, paths, shapes, ids
        ):
            pred_results = []
        ...
        assert (
            trt_batch_size >= self.batch_size
        ), f"The batch size you set is {self.batch_size}, it must <= tensorrt binding batch size {trt_batch_size}."
        tmp = torch.randn(self.batch_size, 3, self.img_size, self.img_size).to(
            self.device
        )
