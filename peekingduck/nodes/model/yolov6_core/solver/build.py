...
def build_optimizer(cfg, model):
    ...
    assert (
        cfg.solver.optim == "SGD" or "Adam"
    ), "ERROR: unknown optimizer, use SGD defaulted"
    if cfg.solver.optim == "SGD":
        optimizer = torch.optim.SGD(
            g_bnw, lr=cfg.solver.lr0, momentum=cfg.solver.momentum, nesterov=True
        )
    elif cfg.solver.optim == "Adam":
        optimizer = torch.optim.Adam(
            g_bnw, lr=cfg.solver.lr0, betas=(cfg.solver.momentum, 0.999)
        )
...
def build_lr_scheduler(cfg, optimizer, epochs):
    ...
    if cfg.solver.lr_scheduler == "Cosine":
        lf = (
            lambda x: ((1 - math.cos(x * math.pi / epochs)) / 2) * (cfg.solver.lrf - 1)
            + 1
        )
    else:
