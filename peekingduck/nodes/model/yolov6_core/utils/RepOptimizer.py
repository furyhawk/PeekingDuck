...
def extract_scales(model):
    ...
    for b in blocks:
        ...
        if hasattr(b, "scale_identity"):
            scales.append(
                (
                    b.scale_identity.weight.detach(),
                    b.scale_1x1.weight.detach(),
...
class RepVGGOptimizer(SGD):
    ...
    def reinitialize(self, scales_by_idx, conv3x3_by_idx, use_identity_scales):
        for scales, conv3x3 in zip(scales_by_idx, conv3x3_by_idx):
            ...
            kernel_1x1 = nn.Conv2d(
                in_channels, out_channels, 1, device=conv3x3.weight.device
            )
            if len(scales) == 2:
                conv3x3.weight.data = conv3x3.weight * scales[1].view(
                    -1, 1, 1, 1
                ...
                identity = torch.from_numpy(
                    np.eye(out_channels, dtype=np.float32).reshape(
                        out_channels, out_channels, 1, 1
                    )
                ).to(conv3x3.weight.device)
                conv3x3.weight.data = conv3x3.weight * scales[2].view(
                    -1, 1, 1, 1
                ) + F.pad(kernel_1x1.weight, [1, 1, 1, 1]) * scales[1].view(-1, 1, 1, 1)
                if (
                    use_identity_scales
                ):  # You may initialize the imaginary CSLA block with the trained identity_scale values. Makes almost no difference.
                    identity_scale_weight = scales[0]
                    conv3x3.weight.data += F.pad(
                        identity * identity_scale_weight.view(-1, 1, 1, 1), [1, 1, 1, 1]
                    )
                else:
    ...
    def generate_gradient_masks(self, scales_by_idx, conv3x3_by_idx, cpu_mode=False):
        ...
        for scales, conv3x3 in zip(scales_by_idx, conv3x3_by_idx):
            para = conv3x3.weight
            if len(scales) == 2:
                mask = torch.ones_like(para, device=scales[0].device) * (
                    scales[1] ** 2
                ).view(-1, 1, 1, 1)
                mask[:, :, 1:2, 1:2] += torch.ones(
                    para.shape[0], para.shape[1], 1, 1, device=scales[0].device
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if p in self.grad_mask_map:
                    d_p = (
                        p.grad.data * self.grad_mask_map[p]
                    )  # Note: multiply the mask here
                else:
                    d_p = p.grad.data
