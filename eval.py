...
from peekingduck.nodes.model.yolov6_core.core.evaler import Evaler
from peekingduck.nodes.model.yolov6_core.utils.events import LOGGER
from peekingduck.nodes.model.yolov6_core.utils.general import increment_name


def get_args_parser(add_help=True):
    parser = argparse.ArgumentParser(
        description="YOLOv6 PyTorch Evalating", add_help=add_help
    )
    parser.add_argument(
...
def run(
    ...
    # init
    val = Evaler(
        data, batch_size, img_size, conf_thres, iou_thres, device, half, save_dir
    )
